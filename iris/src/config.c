...
static int INFLASHFUN user_font_name_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_EDIT)
  {
    ...
    if (getstring(name, 31, false, false, false) == 13)
      font_set_name(fontId, name);
    clearToEndOfLine(row, col + 4);
...
static int INFLASHFUN keyboard_macro_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_EDIT)
  {
    ...
    while (1)
    {
      ...
      if (i >= 0)
        print("\033[%i;3H\033[7m %s \033[27m", 8 + i, keyboard_get_keyname(info.key));
      uint8_t c = waitkey(false);
      if (i >= 0)
        print("\033[%i;3H %s ", 8 + i, keyboard_get_keyname(info.key));
      ...
      else if (c == KEY_ENTER)
      {
        ...
        if (line[0] != 0 || n > 0)
        {
          print("\033[%i;3HPress any key to continue...", n + 9);
          waitkey(false);
        }
...
static int INFLASHFUN keyboard_key_mapping_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_EDIT)
  {
    ...
    while (1)
    {
      ...
      if (printPage)
      {
        ...
        for (int i = 0; i < pageSize; i++)
        {
          ...
          if (p * pageSize + i < totalItems)
          {
            ...
            {
              k++;
            } while (settings.Keyboard.user_mapping[k] == k && k < 256);
          }
        }
      ...
      if (i >= 0)
        print("\033[%i;%iH\033[7m %s \033[27m", 8 + i, 9, keyboard_get_keyname(k));
      uint8_t c = waitkey(false);
      if (i >= 0)
        print("\033[%i;%iH %s ", 8 + i, 9, keyboard_get_keyname(k));
      ...
      else if (c == KEY_INSERT)
      {
        ...
        while (fromKey == HID_KEY_NONE && keyboard_keymap_mapping(&fromKey))
          run_tasks(false);
        ...
        while (keyboard_keymap_mapping(NULL))
          run_tasks(false);
...
{
  if (no <= 4096 / 128)
    memcpy(xmodem_configdata + (no - 1) * 128, charData, 128);
  return true;
}
  ...
  else if (callType == IFT_EDIT)
  {
    ...
    while (1)
    {
      if (printPage)
      {
        ...
        for (int i = 23; i < 30; i++)
          clearToEndOfLine(i, 2);
        printPage = false;
      }

      if (i >= 0)
        print("\033[%i;%iH\033[7m %s \033[27m", firstItemRow + i, 13, get_config_name(i, &header));
      uint8_t c = waitkey(false);
      if (i >= 0)
        print("\033[%i;%iH %s ", firstItemRow + i, 13, header.name);
      ...
      else if ((c == 'n' || c == 'N') && (header.magic == CONFIG_MAGIC))
      {
        ...
        if (getstring(header.name, 63, false, false, false) != 27)
        {
          ...
          if (i == currentConfig)
            memcpy(&settings, &header, sizeof(struct SettingsHeaderStruct));
        }
        printPage = true;
      }
      ...
      else if ((c == '*') && i != currentStartupConfig && (header.magic == CONFIG_MAGIC))
      {
        ...
        if (currentConfig == 0)
          settings.Header.startupConfig = i;
        printPage = true;
      }
...
static int INFLASHFUN color_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  if (color != NULL)
  {
    ...
    else if (callType == IFT_PRINT)
    {
      ...
      for (int c = 0; c < 13; c++)
        framebuf_set_fullcolor(col + 3 + c, row - 1, *color, 0);
    }
    else if (callType == IFT_EDIT)
    {
      while (1)
    ...
    else if (callType == IFT_DEFAULT)
    {
      ...
      else
      {
        switch (id)
        {
        case MI_COLOR_MONO_BG:
          id = MI_COLOR0;
          break;
        case MI_COLOR_MONO_NORM:
          id = MI_COLOR7;
          break;
        case MI_COLOR_MONO_BOLD:
          id = MI_COLOR15;
          break;
        case MI_PCOLOR_MONO_BG:
          id = MI_PCOLOR0;
          break;
        case MI_PCOLOR_MONO_NORM:
          id = MI_PCOLOR15;
          break;
        case MI_PCOLOR_MONO_BOLD:
          id = MI_PCOLOR1;
          break;
        }
...
static int INFLASHFUN ttype_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_EDIT)
  {
    ...
    if (settings.Terminal.ttype != CFG_TTYPE_PETSCII)
    {
      settings.Terminal.bgcolor &= 7;
      settings.Terminal.fgcolor &= 7;
    }
    res = *(item->value) != v;
  }
...
static int INFLASHFUN color16_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_PRINT)
  {
    if (settings.Terminal.ttype == CFG_TTYPE_PETSCII)
    {
      ...
      for (int c = 0; c < strlen(petsciicolornames[*(item->value)]) + 4; c++)
        framebuf_set_fullcolor(col + 2 + c, row - 1, color, 0);
    }
    else
...
static int INFLASHFUN attr_fn(const struct MenuItemStruct *item, int callType, int row, int col)
{
  ...
  else if (callType == IFT_PRINT)
  {
    ...
    if (*(item->value) & ATTR_BOLD)
      print(";1");
    if (*(item->value) & ATTR_UNDERLINE)
      print(";4");
    if (*(item->value) & ATTR_BLINK)
      print(";5");
    if (*(item->value) & ATTR_INVERSE)
      print(";7");
    print("m");
    if (*(item->value) & ATTR_BOLD)
    {
...
static int INFLASHFUN printMenu(const char *title, int col, const struct MenuItemStruct *items, uint8_t numItems, bool haveDefaults)
{
  ...
  if (haveDefaults)
  {
    print("\033[%i;%iHD)  %s", 6 + i, col, getItemLabel(items, numItems, haveDefaults, i));
    i++;
  }
  ...
  while (1)
  {
    ...
    if (c == 13)
    {
      ...
      else if (haveDefaults && i == totalItems - 2)
      {
        ...
        uint8_t c = 0;
        while (c != 'y' && c != 'n' && c != 27)
          c = tolower(waitkey(false));
        print("%c\033[?25l", c);
        if (c == 'y')
          setDefaults(items, totalItems, menuIdPathLen == 0);
