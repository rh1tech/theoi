...
void framebuf_set_fullcolor(uint8_t x, uint8_t y, uint8_t fg, uint8_t bg)
{
  ...
  if (y < num_rows && x < framebuf_get_ncols(y))
  {
    ...
    if (double_size_chars)
      set_fullcolor(MKIDX(x, y + 1), fg, bg);
  }
}
...
void framebuf_scroll_region(uint8_t start, uint8_t end, int8_t n, uint8_t fg, uint8_t bg)
{
  ...
  if (n != 0 && start < num_rows && end < num_rows)
  {
    ...
    if (n > 0)
    {
      ...
      {
        if (!double_size_chars)
          memmove(framebuf_rowattr + yborder + start, framebuf_rowattr + start + yborder + n, end - start + 1 - n);
      ...
      if (n > end - start + 1)
        n = end - start + 1;
      if (!double_size_chars)
        memset(framebuf_rowattr + (end + yborder + 1 - n), 0, n);
      for (int y = 0; y < n; y++)
    ...
    else if (n < 0)
    {
      ...
      {
        if (!double_size_chars)
          memmove(framebuf_rowattr + start + yborder + n, framebuf_rowattr + start + yborder, end - start + 1 - n);
      ...
      if (n > end - start + 1)
        n = end - start + 1;
      if (!double_size_chars)
        memset(framebuf_rowattr + start + yborder, 0, n);
      for (int i = 0; i < n; i++)
...
void framebuf_insert(uint8_t x, uint8_t y, uint8_t n, uint8_t fg, uint8_t bg)
{
  ...
  if (y < num_rows && x < framebuf_get_ncols(y))
  {
    for (int i = 0; i < ((int)num_cols) - (x + n); i++)
    {
      ...
      if (double_size_chars)
        set_char_and_attr(MKIDX(col, y + 1), get_char_and_attr(MKIDX(col - n, y + 1)));
    }
...
void framebuf_delete(uint8_t x, uint8_t y, uint8_t n, uint8_t fg, uint8_t bg)
{
  ...
  if (y < num_rows && x < framebuf_get_ncols(y))
  {
    for (int i = 0; i < ((int)num_cols) - (x + n); i++)
    {
      ...
      if (double_size_chars)
        set_char_and_attr(MKIDX(x + i, y + 1), get_char_and_attr(MKIDX(x + n + i, y + 1)));
    }
...
void framebuf_init(bool forceDVI)
{
  ...
  for (uint32_t GPnn = 6; GPnn < 14; GPnn++)
  {
    gpio_init(GPnn);
    gpio_set_dir(GPnn, false);
  }
  is_dvi = true;
  font_init();
  memset(framebuf_data, 0, sizeof(framebuf_data));
